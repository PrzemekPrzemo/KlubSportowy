...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0"><i class="bi bi-clipboard-data me-1"></i> ISU TES+PCS — Łyżwiarstwo figurowe</h4>
...
<div class="card">
    ...
    <table class="table table-hover mb-0">
        <thead class="table-light">
            <tr>
                <th>Data</th><th>Zawodnik</th><th>Routine</th>
                <th class="text-end">TES</th><th class="text-end">PCS</th><th class="text-end">Deduct.</th>
                <th class="text-end">Total</th><th class="text-end">Miejsce</th><th></th>
            </tr>
        </thead>
        <tbody>
        <?php if (empty($performances)): ?>
            <tr><td colspan="9" class="text-center text-muted py-4">Brak występów.</td></tr>
        <?php else: foreach ($performances as $p): ?>
            <tr>
                ...
                <td class="text-end">
                    <a href="<?= url('figureskating/scoring/' . (int)$p['id']) ?>" class="btn btn-sm btn-outline-primary">
                        <i class="bi bi-eye"></i>
                    </a>
                    <form method="POST" action="<?= url('figureskating/scoring/' . (int)$p['id'] . '/delete') ?>" class="d-inline" onsubmit="return confirm('Usunąć występ?')">
                        <?= csrf_field() ?>
...
<div class="modal fade" id="perfModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <form method="POST" action="<?= url('figureskating/scoring/store') ?>">
        <?= csrf_field() ?>
        <div class="modal-header">
          <h5 class="modal-title">Nowy występ ISU</h5>
        ...
        <div class="modal-body">
          <div class="row g-2">
            ...
            <div class="col-md-4"><label class="form-label">TES (technical)</label><input type="number" step="0.01" name="technical_score" class="form-control"></div>
            <div class="col-md-4"><label class="form-label">PCS (presentation)</label><input type="number" step="0.01" name="presentation_score" class="form-control"></div>
