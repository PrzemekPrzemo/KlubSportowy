-- Dance grace-sports base migration
-- Wzorzec: app/Sports/Gymnastics/migrations/010_grace_base.sql
-- Bardziej ogolne niz DanceSport (taniec towarzyski) — obejmuje hip-hop,
-- balet, jazz, modern, contemporary, ludowy, itp.
SET foreign_key_checks = 0;

CREATE TABLE IF NOT EXISTS `dance_routines` (
    `id`                INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `club_id`           INT UNSIGNED NOT NULL,
    `name`              VARCHAR(120) NOT NULL,
    `discipline`        VARCHAR(60) NOT NULL COMMENT 'np. hip_hop/ballet/jazz/modern/contemporary/folk',
    `difficulty_level`  VARCHAR(40) NULL,
    `duration_seconds`  INT UNSIGNED NULL,
    `description`       TEXT NULL,
    `is_active`         TINYINT(1) NOT NULL DEFAULT 1,
    KEY `idx_dance_routines_club` (`club_id`),
...
CREATE TABLE IF NOT EXISTS `dance_performances` (
    ...
    KEY `idx_dance_perf_member_disc` (`member_id`, `discipline`),
