...
class ResultsController extends BaseController {
    ...
    public function index(): void {
        $this->render('table_tennis/results/index', ['title'=>'Wyniki zawodów — Tenis stołowy','results'=>(new TableTennisResultModel())->listForClub(),'members'=>(new MemberModel())->search('','aktywny',null,1,500)['data']??[],'categories'=>TableTennisResultModel::$CATEGORIES]);
    }
    public function store(): void {
        ...
        (new TableTennisResultModel())->insert(['member_id'=>$memberId,'competition_name'=>trim($_POST['competition_name']??''),'competition_date'=>trim($_POST['competition_date']??'')?:date('Y-m-d'),'age_category'=>trim($_POST['age_category']??'')?:null,'category'=>$category,'placement'=>!empty($_POST['placement'])?(int)$_POST['placement']:null,'notes'=>trim($_POST['notes']??'')?:null]);
