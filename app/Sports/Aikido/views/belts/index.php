...
<div class="card">
    <table class="table table-hover mb-0">
        <thead class="table-light">
            <tr><th>Zawodnik</th><th>Nr</th><th>Pas</th><th>Styl</th><th>Data nadania</th><th>Egzaminator</th><th>Miejsce</th><th></th></tr>
        </thead>
        <tbody>
        <?php if (empty($belts)): ?>
        ...
        <?php else: ?>
            <?php foreach ($belts as $b):
                ...
                $textColor = in_array($bi['color'], ['#000', '#8B4513', '#28a745', '#007bff', '#dc3545', '#ff8c00'], true) ? '#fff' : '#333';
            ?>
                <tr>
                    <td><strong><?= View::e($b['last_name']) ?> <?= View::e($b['first_name']) ?></strong></td>
                    <td class="text-muted small"><?= View::e($b['member_number']) ?></td>
                    <td>
                        <span class="badge" style="background:<?= $bi['color'] ?>;color:<?= $textColor ?>;">
                            <?= View::e($bi['label']) ?>
                        </span>
                    </td>
                    <td class="text-muted small"><?= View::e($styles[$b['style']] ?? $b['style']) ?></td>
                    <td><?= View::e($b['granted_date']) ?></td>
                    <td><?= View::e($b['examiner'] ?? '—') ?></td>
                    <td><?= View::e($b['location'] ?? '—') ?></td>
                    <td>
                        <form method="POST" action="<?= url('aikido/belts/'.(int)$b['id'].'/delete') ?>"
...
<div class="modal fade" id="beltModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="<?= url('aikido/belts/store') ?>">
                <?= csrf_field() ?>
                <div class="modal-header">
                    <h5 class="modal-title"><i class="bi bi-arrows-angle-expand me-1"></i> Nadaj pas</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-success"><i class="bi bi-arrows-angle-expand me-1"></i> Nadaj pas</button>
                </div>
            </form>
        </div>
    </div>
