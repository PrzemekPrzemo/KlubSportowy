...
class AikidoBeltModel extends ClubScopedModel
{
    protected string $table = 'aikido_belts';
    ...
    public static array $STYLES = [
        'aikikai'    => 'Aikikai',
        'ki_society' => 'Ki Society',
        'yoshinkan'  => 'Yoshinkan',
        'iwama'      => 'Iwama Ryu',
        'tomiki'     => 'Tomiki (Shodokan)',
    ];

    public function listForClub(): array
    {
        $clubId = $this->clubId();
        $sql = "SELECT ab.*, m.first_name, m.last_name, m.member_number
                FROM aikido_belts ab
                JOIN members m ON m.id = ab.member_id
                WHERE ab.club_id = ?
                ORDER BY m.last_name, ab.granted_date DESC";
    ...
    public function currentBelt(int $memberId): ?array
    {
        $stmt = $this->db->prepare(
            "SELECT * FROM aikido_belts WHERE club_id = ? AND member_id = ?
