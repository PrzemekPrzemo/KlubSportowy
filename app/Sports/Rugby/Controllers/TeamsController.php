<?php

namespace App\Sports\Rugby\Controllers;

use App\Controllers\BaseController;
use App\Controllers\Traits\RequiresActiveSport;
use App\Helpers\Csrf;
use App\Helpers\Database;
use App\Helpers\Session;
use App\Models\MemberModel;
use App\Sports\Rugby\Models\RugbyTeamModel;

class TeamsController extends BaseController
{
    use RequiresActiveSport;

    public function __construct()
    {
        parent::__construct();
        $this->requireLogin();
        $this->requireClubContext();
        $this->requireSportActive('rugby');
    }

    public function index(): void
    {
        $model = new RugbyTeamModel();
        $teams = $model->listForClub();
        $rosters = [];
        foreach ($teams as $t) $rosters[$t['id']] = $model->roster((int)$t['id']);

        $this->render('rugby/teams/index', [
            'title'      => 'Drużyny rugby',
            'teams'      => $teams,
            'rosters'    => $rosters,
            'members'    => (new MemberModel())->search('', 'aktywny', null, 1, 500)['data'] ?? [],
            'categories' => RugbyTeamModel::$CATEGORIES,
            'formats'    => RugbyTeamModel::$FORMATS,
            'positions'  => RugbyTeamModel::$POSITIONS,
        ]);
    }

    public function store(): void
    {
        Csrf::verify();
        $name = trim($_POST['name'] ?? '');
        if ($name === '') { Session::flash('error', 'Podaj nazwę drużyny.'); $this->redirect('rugby/teams'); }
        $category = array_key_exists($_POST['category'] ?? '', RugbyTeamModel::$CATEGORIES) ? $_POST['category'] : 'senior_m';
        $format   = array_key_exists($_POST['format']   ?? '', RugbyTeamModel::$FORMATS)    ? $_POST['format']   : '15s';

        (new RugbyTeamModel())->insert([
            'name'      => $name,
            'category'  => $category,
            'format'    => $format,
            'age_group' => trim($_POST['age_group'] ?? '') ?: null,
            'coach_id'  => !empty($_POST['coach_id']) ? (int)$_POST['coach_id'] : null,
        ]);
        Session::flash('success', 'Drużyna utworzona.');
        $this->redirect('rugby/teams');
    }

    public function delete(string $id): void
    {
        Csrf::verify();
        (new RugbyTeamModel())->delete((int)$id);
        Session::flash('success', 'Drużyna usunięta.');
        $this->redirect('rugby/teams');
    }

    public function addPlayer(string $teamId): void
    {
        Csrf::verify();
        $memberId = (int)($_POST['member_id'] ?? 0);
        $tid      = (int)$teamId;
        if ($memberId <= 0 || $tid <= 0) { Session::flash('error', 'Wybierz zawodnika.'); $this->redirect('rugby/teams'); }
        $position = array_key_exists($_POST['position'] ?? '', RugbyTeamModel::$POSITIONS) ? $_POST['position'] : 'uniwersalny';

        Database::pdo()->prepare(
            "INSERT IGNORE INTO rugby_players (club_id, team_id, member_id, jersey_number, position, is_captain)
             VALUES (?, ?, ?, ?, ?, ?)"
        )->execute([
            $this->currentClub(), $tid, $memberId,
            !empty($_POST['jersey_number']) ? (int)$_POST['jersey_number'] : null,
            $position, isset($_POST['is_captain']) ? 1 : 0,
        ]);
        Session::flash('success', 'Zawodnik dodany do drużyny.');
        $this->redirect('rugby/teams');
    }

    public function removePlayer(string $id): void
    {
        Csrf::verify();
        Database::pdo()->prepare("DELETE FROM rugby_players WHERE id = ? AND club_id = ?")
            ->execute([(int)$id, $this->currentClub()]);
        Session::flash('success', 'Usunięto z drużyny.');
        $this->redirect('rugby/teams');
    }
}
