...
class ResultsController extends BaseController
{
    ...
    {
        $this->render('taekwondo/results/index', [
            'title'         => 'Wyniki zawodów — Taekwondo',
            'results'       => (new TaekwondoResultModel())->listForClub(),
    ...
    public function store(): void
    {
        ...
        if ($memberId <= 0) {
            ...
            $this->redirect('taekwondo/results');
        }

        $category    = array_key_exists($_POST['category'] ?? '', TaekwondoResultModel::$CATEGORIES)
                            ? $_POST['category'] : null;
        $allWeights  = array_merge(
            TaekwondoResultModel::$WEIGHT_CLASSES_MEN,
            TaekwondoResultModel::$WEIGHT_CLASSES_WOMEN
        );
        $weightClass = in_array($_POST['weight_class'] ?? '', $allWeights, true)
                            ? $_POST['weight_class'] : null;

        (new TaekwondoResultModel())->insert([
            'member_id'        => $memberId,
    ...
    {
        Csrf::verify();
        (new TaekwondoResultModel())->delete((int)$id);
