-- Pilates grace-sports base migration
...
CREATE TABLE IF NOT EXISTS `pilates_routines` (
    ...
    `discipline`        VARCHAR(60) NOT NULL COMMENT 'np. mat/reformer/cadillac/chair/barrel',
    `difficulty_level`  VARCHAR(40) NULL,
    `duration_seconds`  INT UNSIGNED NULL,
    `description`       TEXT NULL,
    `is_active`         TINYINT(1) NOT NULL DEFAULT 1,
    KEY `idx_pilates_routines_club` (`club_id`),
    FOREIGN KEY (`club_id`) REFERENCES `clubs`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `pilates_performances` (
    `id`                INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `club_id`           INT UNSIGNED NOT NULL,
    ...
    KEY `idx_pilates_perf_member_disc` (`member_id`, `discipline`),
    KEY `idx_pilates_perf_club` (`club_id`),
    FOREIGN KEY (`club_id`)   REFERENCES `clubs`(`id`)   ON DELETE CASCADE,
