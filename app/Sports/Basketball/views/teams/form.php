...
$isEdit = !empty($team);
$action = $isEdit ? url('basketball/teams/' . (int)$team['id'] . '/update') : url('basketball/teams/store');
?>
<form method="POST" action="<?= $action ?>" class="card p-4">
    ...
    <div class="mt-4 d-flex gap-2">
        ...
        <a href="<?= url('basketball/teams') ?>" class="btn btn-outline-secondary">Anuluj</a>
        <?php if ($isEdit): ?>
            <form method="POST" action="<?= url('basketball/teams/' . (int)$team['id'] . '/delete') ?>" onsubmit="return confirm('Usunąć?')" class="ms-auto m-0">
