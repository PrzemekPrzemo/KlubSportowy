-- Fitness grace-sports base migration
...
CREATE TABLE IF NOT EXISTS `fitness_routines` (
    ...
    `discipline`        VARCHAR(60) NOT NULL COMMENT 'np. aerobic/step/zumba/hiit/strength_circuit',
    `difficulty_level`  VARCHAR(40) NULL,
    `duration_seconds`  INT UNSIGNED NULL,
    `description`       TEXT NULL,
    `is_active`         TINYINT(1) NOT NULL DEFAULT 1,
    KEY `idx_fitness_routines_club` (`club_id`),
    FOREIGN KEY (`club_id`) REFERENCES `clubs`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `fitness_performances` (
    `id`                INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `club_id`           INT UNSIGNED NOT NULL,
    ...
    KEY `idx_fitness_perf_member_disc` (`member_id`, `discipline`),
    KEY `idx_fitness_perf_club` (`club_id`),
    FOREIGN KEY (`club_id`)   REFERENCES `clubs`(`id`)   ON DELETE CASCADE,
