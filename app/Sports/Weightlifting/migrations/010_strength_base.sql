-- Weightlifting strength-sports base migration
-- Wspolne tabele dla sportow silowych: kategorie wagowe, proby, rekordy zyciowe.
SET foreign_key_checks = 0;

CREATE TABLE IF NOT EXISTS `weightlifting_weight_classes` (
    ...
    KEY `idx_weightlifting_wc_club` (`club_id`),
    FOREIGN KEY (`club_id`) REFERENCES `clubs`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `weightlifting_attempts` (
    `id`              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `club_id`         INT UNSIGNED NOT NULL,
    ...
    KEY `idx_weightlifting_att_member_disc` (`member_id`, `discipline`),
    KEY `idx_weightlifting_att_club` (`club_id`),
    FOREIGN KEY (`club_id`)   REFERENCES `clubs`(`id`)   ON DELETE CASCADE,
    FOREIGN KEY (`member_id`) REFERENCES `members`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `weightlifting_personal_records` (
    `id`              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `club_id`         INT UNSIGNED NOT NULL,
    `member_id`       INT UNSIGNED NOT NULL,
    ...
    `notes`           VARCHAR(255) NULL,
    KEY `idx_weightlifting_pr_member_disc` (`member_id`, `discipline`),
    KEY `idx_weightlifting_pr_club` (`club_id`),
    FOREIGN KEY (`club_id`)   REFERENCES `clubs`(`id`)   ON DELETE CASCADE,
    FOREIGN KEY (`member_id`) REFERENCES `members`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
