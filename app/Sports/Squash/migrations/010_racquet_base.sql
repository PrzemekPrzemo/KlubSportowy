-- Squash racquet-sports base migration
-- Wspolne tabele dla sportow raketkowych: korty, mecze.
SET foreign_key_checks = 0;

CREATE TABLE IF NOT EXISTS `squash_courts` (
    ...
    KEY `idx_squash_courts_club` (`club_id`),
    FOREIGN KEY (`club_id`) REFERENCES `clubs`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `squash_matches` (
    `id`             INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `club_id`        INT UNSIGNED NOT NULL,
    ...
    `notes`          VARCHAR(255) NULL,
    KEY `idx_squash_m_club_date` (`club_id`, `played_at`),
    KEY `idx_squash_m_pa`        (`player_a_id`),
    KEY `idx_squash_m_pb`        (`player_b_id`),
    KEY `idx_squash_m_court`     (`court_id`),
    FOREIGN KEY (`club_id`)     REFERENCES `clubs`(`id`)             ON DELETE CASCADE,
    ...
    FOREIGN KEY (`court_id`)    REFERENCES `squash_courts`(`id`)  ON DELETE SET NULL,
