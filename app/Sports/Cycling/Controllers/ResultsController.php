...
class ResultsController extends BaseController {
    ...
    public function index(): void {
        $this->render('cycling/results/index', ['title'=>'Wyniki zawodów — Kolarstwo','results'=>(new CyclingResultModel())->listForClub(),'members'=>(new MemberModel())->search('','aktywny',null,1,500)['data']??[],'categories'=>CyclingResultModel::$CATEGORIES]);
    }
    public function store(): void {
        ...
        (new CyclingResultModel())->insert(['member_id'=>$memberId,'competition_name'=>trim($_POST['competition_name']??''),'competition_date'=>trim($_POST['competition_date']??'')?:date('Y-m-d'),'age_category'=>trim($_POST['age_category']??'')?:null,'category'=>$category,'placement'=>!empty($_POST['placement'])?(int)$_POST['placement']:null,'notes'=>trim($_POST['notes']??'')?:null]);
