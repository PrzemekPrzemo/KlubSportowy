...
class FieldHockeyTeamModel extends ClubScopedModel
{
    protected string $table = 'field_hockey_teams';
    ...
    public function listForClub(): array
    {
        $stmt = $this->db->prepare(
            "SELECT t.*,
                    (SELECT COUNT(*) FROM field_hockey_players p WHERE p.team_id = t.id) AS player_count,
                    c.first_name AS coach_first, c.last_name AS coach_last
             FROM field_hockey_teams t
    ...
    public function playerTeam(int $memberId): ?array
    {
        $stmt = $this->db->prepare(
            "SELECT t.*, p.position, p.jersey_number, p.is_captain
             FROM field_hockey_teams t
             JOIN field_hockey_players p ON p.team_id = t.id
