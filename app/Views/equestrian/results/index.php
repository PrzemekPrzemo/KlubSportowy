...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0">Wyniki zawodów — Jeździectwo</h4>
...
<div class="card">
    <table class="table table-hover mb-0">
        <thead class="table-light">
            <tr><th>Zawodnik</th><th>Koń</th><th>Zawody</th><th>Data</th><th>Dyscyplina</th><th>Klasa</th><th>Miejsce</th><th>Wynik</th><th>Błędy</th><th></th></tr>
        </thead>
        <tbody>
        ...
        <?php else: ?>
            ...
            ?>
                <tr>
                    ...
                    <td><?= View::e($r['class_level'] ?? '—') ?></td>
                    <td><?= $medal ?> <?= $r['placement'] ? View::e($r['placement']).'.' : '—' ?></td>
                    <td><?= $r['score'] !== null ? View::e(number_format((float)$r['score'], 2)) : '—' ?></td>
                    <td><?= $r['faults'] !== null ? View::e($r['faults']) : '—' ?></td>
                    <td>
                        <form method="POST" action="<?= url('equestrian/results/'.(int)$r['id'].'/delete') ?>"
...
<div class="modal fade" id="resultModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="<?= url('equestrian/results/store') ?>">
                <?= csrf_field() ?>
                ...
                <div class="modal-body">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="horse_id" class="form-select">
                                ...
                                <?php foreach ($horses as $h): ?>
                                    <option value="<?= (int)$h['id'] ?>"><?= View::e($h['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-md-4">
                            <label class="form-label">Klasa / Poziom</label>
                            <input type="text" name="class_level" class="form-control" placeholder="np. L, P, CC1*">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            <label class="form-label">Wynik / Punkty</label>
                            <input type="number" name="score" class="form-control" step="0.01">
                        </div>
                    </div>
                    <div class="row g-2 mb-3">
                        <div class="col-md-6">
                            <label class="form-label">Błędy (skoki)</label>
                            <input type="number" name="faults" class="form-control" step="0.5" min="0">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Czas (sekundy)</label>
                            <input type="number" name="time_seconds" class="form-control" step="0.01" min="0">
                        </div>
                    </div>
                    <div class="mb-3">
