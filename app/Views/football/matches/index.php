...
<div class="card p-3 mb-3">
    <form method="GET" class="row g-2">
        ...
        <div class="col-md-3"><a href="<?= url('football/matches/create') ?>" class="btn btn-success w-100"><i class="bi bi-plus"></i> Nowy mecz</a></div>
    </form>
</div>
<div class="card">
    <table class="table table-hover mb-0">
        <thead class="table-light"><tr><th>Data</th><th>Mecz</th><th>Typ</th><th>Wynik</th><th>Status</th><th></th></tr></thead>
        <tbody>
        ...
        <?php else: foreach ($pagination['data'] as $m): ?>
            <tr>
                ...
                <td>
                    <a href="<?= url('football/matches/' . (int)$m['id']) ?>">
                        <strong><?= View::e($m['home_team_name']) ?></strong> vs <?= View::e($m['away_team']) ?>
                    </a>
                    <?php if ($m['league_round']): ?><small class="text-muted d-block">Kolejka: <?= View::e($m['league_round']) ?></small><?php endif; ?>
                </td>
                ...
                <td class="text-end">
                    <a href="<?= url('football/matches/' . (int)$m['id'] . '/edit') ?>" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a>
                </td>
            </tr>
