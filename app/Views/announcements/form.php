...
<form method="POST" action="<?= $action ?>" class="card p-4">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-2">
            ...
            <select name="priority" class="form-select">
                <?php foreach (['normal','important','urgent'] as $p): ?>
                    <option value="<?= $p ?>" <?= ($announcement['priority'] ?? 'normal') === $p ? 'selected' : '' ?>><?= $p ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-2">
            ...
            <select name="target" class="form-select">
                <?php foreach (['members','staff','all','public'] as $t): ?>
                    <option value="<?= $t ?>" <?= ($announcement['target'] ?? 'members') === $t ? 'selected' : '' ?>><?= $t ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            <label class="form-label">Publikacja od</label>
            <input type="datetime-local" name="publish_from" value="<?= View::e($pubFrom) ?>" class="form-control">
        </div>
        <div class="col-md-4">
            <label class="form-label">Publikacja do</label>
        ...
        <div class="col-12">
            <div class="form-check">
                ...
                <label class="form-check-label" for="pub">Opublikowane (odznacz aby zapisać jako szkic)</label>
            </div>
        </div>
        <div class="col-12">
