    ...
    <table class="table table-hover mb-0">
        ...
        <?php else: ?>
            <?php foreach ($belts as $b):
                ...
                $textColor = in_array($bi['color'], ['#000000', '#8B4513', '#6b3410', '#4a2409', '#cc0000', '#dc3545']) ? '#fff' : '#333';
            ?>
                <tr>
                    ...
                    <td><?= View::e($styles[$b['style']] ?? $b['style']) ?></td>
                    <td><?= View::e($b['granted_date']) ?></td>
                    <td><?= View::e($b['examiner'] ?? '—') ?></td>
                    <td><?= View::e($b['location'] ?? '—') ?></td>
                    <td>
                        <div class="d-flex gap-1">
                            <a href="<?= url('karate/belts/'.(int)$b['id'].'/certificate') ?>" class="btn btn-sm btn-outline-primary" target="_blank"><i class="bi bi-printer"></i></a>
                            <form method="POST" action="<?= url('karate/belts/'.(int)$b['id'].'/delete') ?>"
...
<div class="modal fade" id="beltModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="<?= url('karate/belts/store') ?>">
                <?= csrf_field() ?>
