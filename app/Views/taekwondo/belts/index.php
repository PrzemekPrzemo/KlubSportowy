...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0">Pasy — Taekwondo</h4>
    ...
    <table class="table table-hover mb-0">
        ...
        <?php else: ?>
            <?php foreach ($belts as $b):
                ...
                $dark = in_array($bi['color'], ['#000000', '#c0392b', '#dc3545']);
                $textColor = $dark ? '#fff' : '#333';
                $border = $bi['color'] === '#ffffff' ? 'border:1px solid #ccc;' : '';
            ?>
                <tr>
                    <td><strong><?= View::e($b['last_name']) ?> <?= View::e($b['first_name']) ?></strong></td>
                    <td class="text-muted small"><?= View::e($b['member_number']) ?></td>
                    <td>
                        <span class="badge" style="background:<?= $bi['color'] ?>;color:<?= $textColor ?>;<?= $border ?>">
                            <?= View::e($bi['label']) ?>
                        </span>
                    </td>
                    ...
                    <td>
                        <div class="d-flex gap-1">
                            <a href="<?= url('taekwondo/belts/'.(int)$b['id'].'/certificate') ?>" class="btn btn-sm btn-outline-primary" target="_blank"><i class="bi bi-printer"></i></a>
                            <form method="POST" action="<?= url('taekwondo/belts/'.(int)$b['id'].'/delete') ?>"
...
<div class="modal fade" id="beltModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="<?= url('taekwondo/belts/store') ?>">
                <?= csrf_field() ?>
