...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0"><i class="bi bi-calendar3 text-primary me-2"></i>Mecze — Piłka ręczna</h4>
    ...
    <form method="GET" class="d-flex gap-2">
        ...
        <select name="status" class="form-select form-select-sm">
            ...
            <?php foreach ($statuses as $k => $s): ?>
                <option value="<?= $k ?>" <?= $statusFilter === $k ? 'selected' : '' ?>><?= View::e($s['label']) ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            <thead class="table-light">
                <tr>
                    <th>Data</th><th>Gospodarze</th><th>Wynik</th><th>Goście</th>
                    <th>1. poł.</th><th>Status</th><th>Miejsce</th><th></th>
                </tr>
            </thead>
            <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td class="small text-muted">
                        <?= (int)$m['ht_home'] ?>:<?= (int)$m['ht_away'] ?>
                    </td>
                    <td><span class="badge bg-<?= $si['class'] ?>"><?= View::e($si['label']) ?></span></td>
                    <td class="small"><?= View::e($m['location'] ?? '—') ?></td>
                    <td>
                        <form method="POST" action="<?= url('handball/matches/' . (int)$m['id'] . '/delete') ?>" onsubmit="return confirm('Usunąć?')">
                            <?= csrf_field() ?>
...
<div class="modal fade" id="matchModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="POST" action="<?= url('handball/matches/store') ?>">
                <?= csrf_field() ?>
                ...
                <div class="modal-body">
                    <div class="row g-2">
                        <div class="col-6">
                            <label class="form-label">Gospodarze (drużyna klubu)</label>
                        ...
                        <div class="col-6">
                            <label class="form-label">Goście (nazwa zewnętrzna)</label>
                            <input type="text" name="away_team_name" class="form-control" placeholder="np. ORLEN Wisła Płock">
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-3">
                            ...
                            <input type="number" name="home_score" class="form-control" min="0" max="99" value="0">
                        </div>
                        <div class="col-3">
                            <label class="form-label">Goście</label>
                            <input type="number" name="away_score" class="form-control" min="0" max="99" value="0">
                        </div>
                        <div class="col-3">
                            <label class="form-label">1. poł. Gosp.</label>
                            <input type="number" name="ht_home" class="form-control" min="0" max="99" value="0">
                        </div>
                        <div class="col-3">
                            <label class="form-label">1. poł. Gośc.</label>
                            <input type="number" name="ht_away" class="form-control" min="0" max="99" value="0">
                        </div>
                        <div class="col-6">
