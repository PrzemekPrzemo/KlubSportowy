...
CREATE TABLE IF NOT EXISTS `trainings` (
  ...
  `signup_deadline_hours` INT NOT NULL DEFAULT 2 COMMENT 'cancel deadline before training start',
...
CREATE TABLE IF NOT EXISTS `training_attendees` (
  ...
  `member_id`    INT UNSIGNED NOT NULL,
  `status`       ENUM('zapisany','obecny','nieobecny','spozniony','wypisany',
                       'signed_up','waitlist','attended','absent','cancelled')
                 NOT NULL DEFAULT 'signed_up',
  `signup_source` ENUM('admin','trainer','member_self','recurring') NOT NULL DEFAULT 'admin',
